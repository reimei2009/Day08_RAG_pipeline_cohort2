...
Output:
- Mỗi bài báo được lưu thành 1 file JSON trong data/landing/news/.
- JSON có các trường: url, title, date_crawled, content_markdown.
"""
...
def extract_article_text(soup: BeautifulSoup) -> str:
    """
    Extract nội dung bài báo theo hướng robust:
    - Ưu tiên thẻ article.
    - Nếu không có, lấy các đoạn <p>.
    """
...
def crawl_article(url: str) -> dict[str, Any]:
    ...
    if not body or len(body) < 500:
        raise ValueError(
            f"Nội dung crawl quá ngắn ({len(body)} chars). "
            f"Cần kiểm tra lại URL hoặc selector: {url}"
        )
